...
class Node {
  ...
  void select() {
    ...
    if (show) {
      for (int i = 0; i < links.length; i++) {
        if (links[i] != null) links[i].node.select();
      }
    }
  }
  void deselect() {
    ...
    if (show) {
      for (int i = 0; i < links.length; i++) {
        if (links[i] != null) links[i].node.deselect();
      }
    }
  }
  ...
  void display(float _x, float _y, float a, float t, float k, float v) {
    ...
    if (show) {
      ...
      for (int i = 0; i < links.length; i++) {
        stroke(0,30);
        if (i == 0 && !(!isInsert && showDead)) {
          // Support line
          strokeWeight(2);
          line(x, y, x+(r-5)*cos(b+d*i), y+(r-5)*sin(b+d*i)); // start line
        }
        if (links[i] == null) {
          // Free POI
          strokeWeight(1);
          stroke(SESSION[session%SESSION.length],66);
          if (selected) stroke(220,200,0,66);
          line(x+(r-5)*cos(b+d*i), y+(r-5)*sin(b+d*i), x+r*cos(b+d*i), y+r*sin(b+d*i));
        } else {
          // Insertion or deletion node ahead
          strokeWeight(1);
